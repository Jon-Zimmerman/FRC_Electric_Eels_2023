...
public class GetOnChargeStationFromGrid extends CommandBase {
  ...
  private final Translation2d approachTranslation = new Translation2d(-1.0*invertDirectionScalar, 0); // meters per second;
  private final Translation2d balancingTranslation = new Translation2d(-0.3*invertDirectionScalar, 0); // meters per second;
  ...
  @Override
  public void execute() {
    roll = invertDirectionScalar*-m_swerve.gyroInputs.rollDegrees;
    if(!failed){
      if (!onRamp ) {
